...
def main():
    ...
    for i in range(EVENTS):
        engine.publish(Event(event_id=f"bench-{i}", key=f"customer-{i % 1000}", event_type="benchmark", payload={"sequence": i}))
    ...
    print(f"p50_latency_ms={percentile(.50):.4f}")
    print(f"p95_latency_ms={percentile(.95):.4f}")
    print(f"p99_latency_ms={percentile(.99):.4f}")
    engine.shutdown()
